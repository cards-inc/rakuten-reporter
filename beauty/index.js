...
// 設定
// ============================
const SPREADSHEET_ID = '1YyCNm_Y15yEjqu0fGYOvcdrFxoPqagAXyRt0Qci_iEo';
...
functions.http('fetchBeautyReport', async (req, res) => {
  // ダッシュボードモード
  ...
  try {
    ...
    const rmsUser = process.env.BEAUTY_RMS_LOGIN_ID || process.env.RMS_LOGIN_ID;
    const rmsPass = process.env.BEAUTY_RMS_PASSWORD || process.env.RMS_PASSWORD;
    const rakutenPass = process.env.BEAUTY_RMS_2FA_PASSWORD || process.env.RMS_2FA_PASSWORD;
      ...
      if (isAfiLoginPage) {
        ...
        if (hasRLoginForm) {
          ...
          if (needsStep2) {
            ...
            const rakutenEmail = process.env.BEAUTY_RMS_2FA_EMAIL || process.env.RMS_2FA_EMAIL;
...
async function writeRawToSheet(headers, dataRows, sheetName, keyColumnNames) {
  ...
  // 新データをマージ（同キーは上書き、データが充実している方を採用）
  let newCount = 0;
  let updateCount = 0;
...
async function generateDashboardHtml() {
  ...
  // RPP item data by month (商品管理番号で重複排除)
  ...
  // RPP kw data by month (商品管理番号+キーワードで重複排除)
  const rppKwByMonth = {};
  const rppKwSeen = {};
...
<header class="app-header" onclick="location.reload()" style="cursor:pointer">
  <div class="header-inner">
    ...
    <div class="header-meta">BEAUTY｜更新: ${safe(dateStr)}</div>
  </div>
</header>
